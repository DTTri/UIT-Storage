...
UPDATE SANPHAM1 SET GIA*=1.05
WHERE NUOCSX = 'Thai Lan'
GO

--4.Cập nhật giá giảm 5% đối với những sản phẩm do “Trung Quoc” sản xuất có giá từ 10.000 trở xuống (cho quan hệ SANPHAM1).\
UPDATE SANPHAM1 SET GIA*=0.95
...
ALTER TABLE HOCVIEN
ADD CONSTRAINT CK_TUOIHV CHECK(GETDATE() - NGSINH >= 18)
GO

--12. Giảng dạy một môn học ngày bắt đầu (TUNGAY) phải nhỏ hơn ngày kết thúc (DENNGAY).
ALTER TABLE GIANGDAY
ADD CONSTRAINT CK_GIANGDAY CHECK(TUNGAY < DENNGAY)
GO
--13.Giáo viên khi vào làm ít nhất là 22 tuổi
ALTER TABLE GIAOVIEN
ADD CONSTRAINT CK_TUOIGV CHECK(GETDATE() - NGSINH >=22)
GO
...
GO
-- Em da INSERT DU LIEU VAO BANG MONHOC TRUOC DO, NEN KHI THEM CONSTRAINT CK_TC BI CONFLICTED DO CO VAI MON SO TIN CHI LECH QUA 3
-- Em da dung lenh DELETE FROM MONHOC de xoa het du lieu, roi ADD lai constraint CK_TC, sau do insert lai du lieu nhung co vai` row khong the insert vi` vi pham CK_TC
...
SELECT MASP, TENSP
FROM dbo.SANPHAM
WHERE (GIA BETWEEN 30000 AND 40000) AND (NUOCSX = 'Trung Quoc' OR NUOCSX='Thai Lan')
GO
...
FROM DBO.HOADON AS H, DBO.KHACHHANG AS K
WHERE H.MAKH= K.MAKH AND H.NGHD='1/1/2007'


--9.	In ra số hóa đơn, trị giá các hóa đơn do nhân viên có tên “Nguyen Van B” lập trong ngày 28/10/2006.
SELECT H.SOHD, H.TRIGIA
FROM DBO.HOADON AS H, DBO.NHANVIEN AS N
WHERE H.MANV=N.MANV AND N.HOTEN='Nguyen Van B'  AND H.NGHD = '28/10/2006'


--10.	In ra danh sách các sản phẩm (MASP,TENSP) được khách hàng có tên “Nguyen Van A” mua trong tháng 10/2006.
SELECT S.MASP, S.TENSP
FROM DBO.KHACHHANG AS K, DBO.HOADON AS H, DBO.CTHD AS C, DBO.SANPHAM AS S
WHERE H.MAKH=K.MAKH AND H.SOHD=C.SOHD AND C.MASP = S.MASP AND K.HOTEN='Nguyen Van A' AND  (MONTH(NGHD) = 10 AND YEAR(NGHD) = 2006)

--11.	Tìm các số hóa đơn đã mua sản phẩm có mã số “BB01” hoặc “BB02”.
SELECT H.SOHD
FROM DBO.HOADON AS H, DBO.CTHD AS C
WHERE H.SOHD=C.SOHD AND (C.MASP='BB01' OR C.MASP='BB02')

---------------- Bai tap 6 ----------------
--1.	In ra danh sách (mã học viên, họ tên, ngày sinh, mã lớp) lớp trưởng của các lớp.
SELECT H.MAHV, H.HO +' '+ H.TEN AS HOTEN, H.NGSINH, H.MALOP
FROM DBO.LOP AS L, DBO.HOCVIEN AS H
...
FROM DBO.HOCVIEN AS H, DBO.KETQUATHI AS K
WHERE K.MAHV=H.MAHV AND K.MAMH='CTRR' AND H.MALOP='K12'
...
FROM DBO.KETQUATHI AS K, DBO.HOCVIEN AS H
WHERE K.MAHV= H.MAHV
GROUP BY K.MAHV, H.HO, H.TEN, K.MAMH, K.KQUA
HAVING MAX(LANTHI)=1 AND K.KQUA='DAT'

--4.	In ra danh sách học viên (mã học viên, họ tên) của lớp “K11” thi môn CTRR không đạt (ở lần thi 1).
SELECT H.MAHV, (H.HO+' '+H.TEN) AS HOTEN
FROM DBO.KETQUATHI AS K, DBO.HOCVIEN AS H
WHERE K.MAHV= H.MAHV AND K.MAMH='CTRR' AND H.MALOP='K11' AND K.KQUA='Khong Dat' AND K.LANTHI=1

--5.	Danh sách học viên (mã học viên, họ tên) của lớp “K” thi môn CTRR không đạt (ở tất cả các lần thi).
/* Cach nay em nghi la sai
SELECT K.MAHV, (H.HO+' '+H.TEN) AS 'HOTEN', MAX(K.LANTHI) AS 'So lan thi'
FROM DBO.KETQUATHI AS K, DBO.HOCVIEN AS H
WHERE K.MAHV=H.MAHV AND H.MALOP='K11'  AND K.KQUA='Khong dat'
GROUP BY K.MAHV, H.HO, H.TEN, H.MALOP, K.MAMH
HAVING K.MAMH='CTRR'
*/
SELECT A.MAHV, HOTEN FROM (
	SELECT KQ.MAHV, (HO + ' ' + TEN) AS HOTEN, LANTHI
	FROM KETQUATHI AS KQ INNER JOIN HOCVIEN AS HV 
	ON KQ.MAHV = HV.MAHV
